   ...
   if [ "$subtype" != "" ] ; then
    for prop in $props; do
     ...
     if [ "" != "$value" ];then
        ...
        read head other <<< $(echo "$value" | sed 's|_| |');
...
#ifdef VENDOR_EDIT
#Zhenkun.Huang@RM.AD.Stability, 2019/12/30, Add for gather system information when system server watchdog timeout
function systrace_sched() {
...
function system_server_half_timeout() {
    system_server_pid=`pgrep system_server`
    swt_path=${DATA_LOG_PATH}/swt_logs/systemserver_pid${system_server_pid}_${CURTIME}
    setprop sys.swt.dir ${swt_path}

    if [ ! -d ${swt_path} ];then
        mkdir -p ${swt_path}
    fi

    strace -ttTf -o ${swt_path}/strace.txt -p ${system_server_pid} &

    systrace_sched 1
    cat /sys/kernel/debug/tracing/trace_pipe > ${swt_path}/atrace.txt &

    vmstat 1 15 > ${swt_path}/vmstat.txt &

    procrank -op > ${swt_path}/procrank_half.txt &

    cat /proc/meminfo > ${swt_path}/meminfo_half.txt &

    iotop -m 10 -n 5 -d 2 > ${swt_path}/iotop.txt &

    sleep 20

    pgrep strace -U root -L 9

    systrace_sched 0
}

function system_server_timeout() {
    swt_path=`getprop sys.swt.dir`
    ...
    if [ "${2}" == "true" ];then
        ...
        procrank -op > ${swt_path}/procrank_kill.txt &

        cat /proc/meminfo > ${swt_path}/meminfo_kill.txt &

        df /data >  ${swt_path}/df_data.txt &

        du -a /data > ${swt_path}/du_data.txt &

        ps -wAT > ${swt_path}/ps.txt &

        cp -rf /d/ion ${swt_path}/ &
        cp -rf /d/dma_buf ${swt_path}/ &
    else
        rm -rf ${swt_path}
    fi
}
...
#ifdef VENDOR_EDIT
#Zhenkun.Huang@RM.AD.Stability, 2019/12/30, Add for gather system information when system server watchdog timeout
        "system_server_half_timeout")
        system_server_half_timeout
    ;;
        "system_server_timeout")
        system_server_timeout $@
    ;;
#endif /* VENDOR_EDIT */
       *)
